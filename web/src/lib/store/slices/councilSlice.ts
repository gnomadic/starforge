...
export const createCouncilSlice = (set: any, get: any): CouncilState => ({
  ...
  contributeToProject: (projectId, resourceId, amount) => {
    const project = get().councilProjects.find(p => p.id === projectId);
    ...
    const councilProjects = get().councilProjects.map(p => {
      if (p.id === projectId) {
        const newProgress = { ...p.currentProgress };
        ...
        for (const [reqResource, reqAmount] of Object.entries(p.resourceRequirements)) {
          if ((newProgress[reqResource] || 0) < reqAmount) {
            isComplete = false;
            break;
          }
        }
